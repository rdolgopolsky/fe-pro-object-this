  ...
  describe('checks methods', () => {
    ...
    it('checkInGuest()', () => {
      ...
      expect({ ...guest }).toStrictEqual({
        firstName: 'Bohdan', lastName: 'Onatskyi', money: 20
      });
      ...
      expect(guestWithoutMoney).toStrictEqual('Sorry, you have not enough money');
      expect(hotel.getLength()).toStrictEqual(1);
      expect(hotel.bankAccount).toStrictEqual(20);
      ...
      expect(hotel.checkInGuest('Man', 'WithMillionDollar', 1000000)).toStrictEqual('Sorry, we have not free spaces');
    });
  });
});
